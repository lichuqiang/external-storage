...
func (r *CapacityReconciler) reconcileWorker() {
	for className := range r.ProvisionSourceMap {
		...
		workFunc := func() bool {
			...
			recordedCapacity, ok := classStatus.Capacity.Capacities[r.Node.Name]
			if ok {
			...
			classStatus.Capacity.Capacities[r.Node.Name] = actualCapacity
			if _, err := r.APIUtil.UpdateStorageClass(class); err != nil {
